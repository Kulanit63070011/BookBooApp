...
const CreateEventPostScreen = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let eventImage = null;
      if (eventImage) {
        // Code to upload event image
        // Example: eventImage = await uploadImage(eventImage);
      }

      // Add event document to Firestore
      const eventDocRef = await addDoc(collection(db, 'events'), { ...eventData, eventImage });

      // Add items to the items collection
      for (const item of items) {
        if (item.name && item.price) { // Only add items that have a name and price
          let itemImageUrl = null;
          if (item.image) {
            // Code to upload item image
            // Example: itemImageUrl = await uploadImage(item.image);
          }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {renderDatePicker(startDate, setStartDate, showStartDatePicker, setShowStartDatePicker)}

      <Text>End Date</Text>
      {renderDatePicker(endDate, setEndDate, showEndDatePicker, setShowEndDatePicker)}
      ...
      <TouchableOpacity onPress={pickEventImage}>
        <View style={styles.imagePicker}>
          {eventImage ? (
            ...
            <Text style={styles.imagePlaceholder}>Tap to select event image</Text>
          )}
        </View>
      </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => pickItemImage(index)}>
            <View style={styles.imagePicker}>
              {item.image ? (
                ...
                <Text style={styles.imagePlaceholder}>Tap to select item image</Text>
              )}
            </View>
          </TouchableOpacity>
        </View>
